...
function StatusBadge({ status, error }: { status: number | null; error: string | null }) {
  ...
  // gRPC OK = 0, HTTP success < 300
  const cls = status === 0 || (status > 0 && status < 300) ? 'online' : status < 500 ? 'provisioning' : 'offline';
  return <span className={`status ${cls}`}>{status === 0 ? 'OK' : status}</span>;
}
...
function MethodBadge({ method }: { method: string }) {
  const colors: Record<string, string> = {
    ...
    'gRPC (query)': 'var(--color-accent-cyan)',
    'gRPC (mutation)': 'var(--color-success)',
  };
  return (
    <span style={{
