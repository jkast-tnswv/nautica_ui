...
import { Table, Cell, SelectField, FormField, FormDialog, Card, InfoSection, Button, RefreshButton, ErrorMessage } from '@components';
...
export function ShipwrightJobs() {
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      filterValue: (j: ShipwrightJobResponse) => shipwrightJobTypeLabel(j.shipwrightJobType),
    },
    {
      header: 'Owner',
    ...
    },
    {
      ...
      filterValue: (j: ShipwrightJobResponse) => shipwrightJobStatusLabel(j.shipwrightJobStatus),
    },
    {
      header: 'Created',
      accessor: (j: ShipwrightJobResponse) => formatTimestamp(j.timestampJobCreated),
      filterValue: (j: ShipwrightJobResponse) => formatTimestamp(j.timestampJobCreated),
      width: '180px',
    },
    {
      header: 'Ended',
      accessor: (j: ShipwrightJobResponse) => formatTimestamp(j.timestampJobEnded),
      filterValue: (j: ShipwrightJobResponse) => formatTimestamp(j.timestampJobEnded),
      width: '180px',
    },
  ], []);

  if (error && jobs.length === 0) {
    return <ErrorMessage error={error} prefix="Error loading jobs" />;
  }
  ...
  return (
    <Card
      ...
      headerAction={
        <>
          <Button size="sm" icon="add" onClick={() => setShowCreateDialog(true)}>
            New Job
          </Button>
