...
import { Table, Cell, Card, InfoSection, Button, ErrorMessage } from '@components';
...
export function LedgerSearch() {
  ...
  return (
    <Card
      ...
      headerAction={
        <form onSubmit={handleSubmit} className="ledger-search-form">
          ...
          <Button type="submit" size="sm" icon="search" disabled={!searchInput.trim() || loading}>
            Search
          </Button>
          {records.length > 0 && (
            <Button type="button" size="sm" variant="secondary" onClick={() => { clear(); setSearchInput(''); }}>
              Clear
            </Button>
          )}
        </form>
