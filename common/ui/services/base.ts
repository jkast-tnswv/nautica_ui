import { getTransport } from './grpc-transport';
...
// gRPC Base Service — wraps ConnectRPC clients with the same
// cross-cutting concerns as BaseService (retry, history, inflight)
export class GrpcBaseService {
  protected createGrpcClient<T extends ServiceType>(service: T): PromiseClient<T> {
  ...
  private isRetryableGrpcError(error: unknown): boolean {
    if (error instanceof Error) {
      ...
      // Retry on unavailable, resource exhausted, internal
      if (msg.includes('[unavailable]') || msg.includes('[resource_exhausted]') || msg.includes('[internal]')) {
        return true;
      }
    }
    return false;
  ...
  private async retryWithBackoff<T>(
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        lastError = error;
        if (!this.isRetryableGrpcError(error) || attempt === retries) {
          throw error;
        }
        const delay = baseDelay * Math.pow(2, attempt) * (0.5 + Math.random() * 0.5);
        await new Promise(resolve => setTimeout(resolve, delay));
  ...
  protected async call<TRes>(
    ...
    const entry: ApiHistoryEntry = {
      id: ++historyIdCounter,
      method: isMutation ? 'gRPC (mutation)' : 'gRPC (query)',
      url: path,
      path,
      queryParams: {},
      status: null,
    ...
    if (isMutation) changeInflight(1);

    try {
      const result = await this.retryWithBackoff(fn);
      entry.durationMs = Date.now() - start;
      entry.status = 0; // gRPC OK
